/**
 * CLI Audit Runner — run from apps/api directory
 * Usage: tsx scripts/run-audit.ts --url "https://example.com" --viewport both
 */
...
import {
  ...
  BidiCheck,
  FontFallbackCheck,
} from '../src/modules/audit/checks/index.js';
import type { BaseCheck } from '../src/modules/audit/checks/base.check.js';
...
import scoringWeightsData from '../../../packages/audit-rules/scoring-weights.json' with { type: 'json' };
...
if (urlIndex === -1 || !args[urlIndex + 1]) {
  console.error('Usage: tsx scripts/run-audit.ts --url <URL> [--viewport desktop|mobile|both]');
  process.exit(1);
}
...
async function runAuditForViewport(url: string, vp: 'desktop' | 'mobile'): Promise<IAuditResult> {
  ...
  try {
    ...
    for (const check of checks) {
      ...
      try {
        ...
        console.error(`\n  ⚠️  Check "${check.category}" failed: ${String(err)}`);
        const maxScore = getMaxScore(check.category as CheckCategory);
        categories.push({
          category: check.category as CheckCategory,
          score: 0,
...
function printReport(result: IAuditResult): void {
  ...
  for (const cat of result.categories) {
    ...
    if (cat.issues.length > 3) {
      console.log(`  ${DIM}  ... and ${cat.issues.length - 3} more${RESET}`);
    }
    if (cat.fixes[0]) {
      console.log(`  ${DIM}Fix: ${cat.fixes[0].description}${RESET}`);
    }
    console.log();
  }

  const allFixes = result.categories.flatMap((c) => c.fixes);
